...
internal partial class Program
{
    ...
    public void ShowInterface()
    {
        ...
        if (_mainForm == null || _mainForm.IsDisposed)
        {
            ...
            if (Directory.Exists(Program.ModulesPath))
            {
                var dlls = Directory.GetFiles(Program.ModulesPath, "*.dll", SearchOption.AllDirectories);
                var loadedModules = ModuleManager.GetAll().ToList();

                foreach (var path in dlls)
                {
                    var instance = ModuleManager.GetModuleByPath(path);

                    _mainForm.RegisterModuleButton(path, Path.GetFileNameWithoutExtension(path), instance);
                }
            }
